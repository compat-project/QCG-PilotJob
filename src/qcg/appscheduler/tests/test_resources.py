...
from qcg.appscheduler.resources import Node, ResourcesType, Resources
from qcg.appscheduler.resources import CRType, CR, CRBind
...
#    print(res.toJSON())
    ...
    r_copy = Resources.fromDict(r.toDict())
    assert all((r_copy != None, r_copy.binding == False, r_copy.rtype == ResourcesType.LOCAL,
        r_copy.nNodes() == 2, r_copy.totalNodes == 2, r_copy.totalCores == r_copy.freeCores == n1Tot + n2Tot, r_copy.usedCores == 0))
    ...
    assert all((len(r_copy.toJSON()) > 0, r_copy.toJSON() == r.toJSON()))
