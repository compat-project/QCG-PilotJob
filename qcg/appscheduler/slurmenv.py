...
def parse_nodelist(nodespec):
	...
	ex_code = p.wait()
	if ex_code != 0:
		raise SlrmEnvError("scontrol command failed: %s" % stderr)
...
def parse_slurm_resources():
	...
	if len(node_names) != len(cores_num):
		raise SlurmEnvError("failed to parse slurm env: number of nodes (%d) mismatch number of cores (%d)" % (len(nodes), len(cores)))
